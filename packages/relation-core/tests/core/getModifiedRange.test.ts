...
describe("getModifiedRange", function() {
  it("getModifiedRange should correct", function() {
  ...
  it("add lines should correct", function() {
    const oldContent = `test1
  test2
  `;
  ...
  `;
    ...
    expect(getModifiedRange(linesRelation, [1, 2])).to.be.eql([1, 2]);
    expect(getModifiedRange(linesRelation, [1, 3])).to.be.eql([1, 5]);
    expect(getModifiedRange(linesRelation, [2, 3])).to.be.eql([4, 5]);
  });
});
